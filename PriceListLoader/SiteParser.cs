...
namespace PriceListLoader {
    class SiteParser {
		private readonly HtmlAgility _htmlAgility = new HtmlAgility();
		...
		public string ParseSelectedSite(SiteInfo siteInfo, bool isAutoMode = false) {
			...
			if (this.siteInfo.Name == SiteInfo.SiteName.msk_invitro_ru ||
				...
				this.siteInfo.Name == SiteInfo.SiteName.yekuk_invitro_ru ||
				...
				if (isAutoMode) {
					_backgroundWorker.ReportProgress(0, "Требуется выбрать файл для обработки");
					return string.Empty;
				}
			...
			switch (this.siteInfo.Name) {
				...
				case SiteInfo.SiteName.krd_clinic23_ru_lab:
				...
				case SiteInfo.SiteName.spb_dcenergo_ru:
				case SiteInfo.SiteName.spb_starsclinic_ru:
				case SiteInfo.SiteName.spb_clinica_blagodat_ru:
				case SiteInfo.SiteName.spb_med_vdk_ru:
				case SiteInfo.SiteName.krd_vrukah_com:
				case SiteInfo.SiteName.kazan_kazan_clinic_ru:
				case SiteInfo.SiteName.sochi_medbr_ru:
				...
				case SiteInfo.SiteName.yekuk_invitro_ru:
				...
				case SiteInfo.SiteName.yekuk_mfcrubin_ru:
					ItemServiceGroup itemServiceGroup = new ItemServiceGroup() { Link = siteInfo.UrlServicesPage };
					ParseSiteYekukMfcrubinRu(docServices, ref itemServiceGroup);
				...
				case SiteInfo.SiteName.spb_clinic_complex_ru:
					ParseSiteSpbClinicComplexTitlePage(docServices);
					break;
				case SiteInfo.SiteName.spb_reasunmed_ru:
					ParseSiteSpbReasunmedRu(docServices);
					break;
				case SiteInfo.SiteName.spb_allergomed_ru:
					ParseSiteSpbAllergomedRu(docServices);
					break;
				case SiteInfo.SiteName.sochi_5vrachey_com:
					ParseSiteSochi5vracheyCom(docServices);
					break;
				case SiteInfo.SiteName.sochi_mc_daniel_ru:
					ParseSiteSochiMcDanielRu(docServices);
					break;
				default:
					break;
			}
			...
			if (this.siteInfo.ServiceGroupItems.Count == 0) {
				_backgroundWorker.ReportProgress((int)progressCurrent, App.errorPrefix + "Не удалось найти ни одной группы услуг");
				return string.Empty;
			}

			return NpoiExcel.WriteItemSiteDataToExcel(this.siteInfo, _backgroundWorker, progressCurrent, progressTo);
		}


		private void ParseSiteSochiMcDanielRu(HtmlDocument docServices) {
			...
			if (nodeCollection != null) {
				foreach (HtmlNode nodeServiceRoot in nodeCollection) {
					...
					ItemServiceGroup serviceGroup = new ItemServiceGroup {
					...
					foreach (HtmlNode nodeService in nodeCollectionServices) {
						...
						for (int i = values.Length - 1; i >= 0; i--) {
							string value = SiteInfo.ClearString(values[i]).Replace("₽", "").TrimStart(' ').TrimEnd(' ');
							if (int.TryParse(value, out int priceValue)) {
								price = SiteInfo.ClearString(values[i]);
								break;
							}
						}
						...
						string name = SiteInfo.ClearString(nodeService.InnerText).Replace(price, "").TrimStart(' ').TrimEnd(' ');
		...
		private void ParseSiteSochi5vracheyCom(HtmlDocument docServices) {
			...
			HtmlDocument docManuallyLoaded = _htmlAgility.GetDocument(url, SiteInfo.SiteName.sochi_5vrachey_com);
		...
		private void ParseSiteSpbAllergomedRu(HtmlDocument docServices) {
			HtmlNodeCollection nodeCollection = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			if (nodeCollection != null) {
				foreach (HtmlNode nodeServiceRoot in nodeCollection) {
					...
					foreach (HtmlNode nodeService in nodeCollectionServices) {
						...
						string name = SiteInfo.ClearString(nodeName.InnerText);
						string price = SiteInfo.ClearString(nodePrice.InnerText);
		...
		private void ParseSiteSpbReasunmedRu(HtmlDocument docServices) {
			HtmlNodeCollection nodeCollection = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			if (nodeCollection == null) {
			...
			foreach (HtmlNode nodeChild in nodeCollection[0].ChildNodes) {
				if (nodeChild.Name.Equals("h3")) {
					...
					foreach (HtmlNode nodeData in htmlNodeTbody) {
						List<ItemService> serviceItems = ReadTrNodes(nodeData);
		...
		private void ParseSiteYekukMfcrubinRu(HtmlDocument docServices, ref ItemServiceGroup itemServiceGroup) {
			HtmlNodeCollection nodeCollection = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			if (nodeCollection == null) {
				Console.WriteLine("nodeCollection == null");
				return;
			...
			foreach (HtmlNode nodeService in nodeCollection) {
				...
				ItemService itemService = new ItemService() {
					Name = SiteInfo.ClearString(nodeName.InnerText),
			...
			HtmlDocument docNextPage = _htmlAgility.GetDocument(nextPageUrl, siteInfo.Name);
			if (docNextPage != null)
		...
		private void ParseSiteSochiMedcentrSochiRu(HtmlDocument docServices) {
			...
			foreach (HtmlNode nodeChild in nodeCollection[0].ChildNodes) {
				if (nodeChild.Name.Equals("h3")) {
					...
					List<ItemService> serviceItems = ReadTrNodes(htmlNodeTbody, 1, 1);
					itemServiceGroup.ServiceItems.AddRange(serviceItems);
					siteInfo.ServiceGroupItems.Add(itemServiceGroup);
				}
			}
			...
			if (nodeCollection != null) {
				foreach (HtmlNode nodeLi in nodeCollection) {
					...
					foreach (HtmlNode nodeTbody in htmlNodeTbodies) {
						List<ItemService> serviceItems = ReadTrNodes(nodeTbody);
						itemServiceGroup.ServiceItems.AddRange(serviceItems);
					}
		...
		private void ParseSiteUfaMamadetiRu(HtmlDocument docServices) {
			...
			foreach (HtmlNode nodeGroup in nodeCollectionServices) {
				...
				foreach (HtmlNode nodeTable in nodeCollectionTables) {
					List<ItemService> serviceItems = ReadTrNodes(nodeTable);
					itemServiceGroup.ServiceItems.AddRange(serviceItems);
				}
			...
			foreach (HtmlNode nodeGroup in nodeCollectionServices) {
				...
				foreach (HtmlNode nodeLi in nodeCollectionLi) {
					try {
						...
						List<ItemService> serviceItems = ReadTrNodes(nodeTableBody);
						itemServiceGroup.ServiceItems.AddRange(serviceItems);
		...
		private void ParseSiteInvitroRU(HtmlDocument docServices) {
            siteInfo.ServiceGroupItems.Add(new ItemServiceGroup() { Link = siteInfo.UrlServicesPage });
			HtmlNodeCollection nodeCollectionServices = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			...
			if (nodeCollectionServices != null) {
                foreach (HtmlNode node in nodeCollectionServices) {
                    ...
                    ItemService itemService = new ItemService() {
                        Name = SiteInfo.ClearString(nodeName.InnerText),
                        Price = SiteInfo.ClearString(node.Attributes["data-prices"].Value)
                    };
                    ...
                    //List<ItemService> serviceItems = ReadTrNodes(node, 1, 2);
                    //itemServiceGroup.ServiceItems.AddRange(serviceItems);
                    //siteInfo.ServiceGroupItems.Add(itemServiceGroup);
                }
            }
				...
				try {
					...
					foreach (HtmlNode nodeGroup in nodeCollectionGroups) {
						...
						foreach (HtmlNode nodeTable in nodeTables) {
							List<ItemService> serviceItems = ReadTrNodes(nodeTable.LastChild);
		...
		private void ParseSiteSpbClinicComplexTitlePage(HtmlDocument docServices) {
			HtmlNodeCollection nodeCollectionServicesRoot = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			...
			foreach (HtmlNode nodeServiceRoot in nodeCollectionServicesRoot) {
				...
				string title = SiteInfo.ClearString(htmlNodeTitle.InnerText);
				_backgroundWorker.ReportProgress(0, "Группа услуг - " + title);
		...
		private void ParseSiteWithLinksOnMainPage(HtmlDocument docServices, bool isFirstCycle = true, string itemGroupNamePrefix = "") {
			HtmlNodeCollection nodeCollectionServices = _htmlAgility.GetNodeCollection(docServices, siteInfo.XPathServices);
			if (nodeCollectionServices == null) {
				Console.WriteLine("nodeCollectionServices is null");
				return;
			...
			foreach (HtmlNode servicePage in nodeCollectionServices) {
				...
				try {
					...
					if (siteInfo.Name == SiteInfo.SiteName.spb_clinic_complex_ru && !string.IsNullOrEmpty(itemGroupNamePrefix))
					...
					switch (siteInfo.Name) {
						...
						case SiteInfo.SiteName.yekuk_immunoresurs_ru:
							...
							break;
						case SiteInfo.SiteName.kazan_ava_kazan_ru:
							ParseSiteKazanAvaKazanRu(docService, ref itemServiceGroup, isFirstCycle);
							break;
						case SiteInfo.SiteName.kazan_mc_aybolit_ru:
						...
						case SiteInfo.SiteName.kovrov_clinicalcenter_ru:
							...
							break;
						case SiteInfo.SiteName.spb_dcenergo_ru:
							ParseSiteSpbDcenergoRu(docService, ref itemServiceGroup);
							break;
						case SiteInfo.SiteName.spb_starsclinic_ru:
							ParseSiteSpbStarsclinicRu(docService, ref itemServiceGroup);
							break;
						case SiteInfo.SiteName.spb_clinica_blagodat_ru:
							ParseSiteSpbClinicaBlagodatRu(docService, ref itemServiceGroup);
							break;
						case SiteInfo.SiteName.spb_med_vdk_ru:
							ParseSiteSpbMedVdkRu(docService, ref itemServiceGroup, isFirstCycle);
							break;
						case SiteInfo.SiteName.krd_vrukah_com:
							ParseSiteKrdVrukahCom(docService, ref itemServiceGroup);
							break;
						case SiteInfo.SiteName.kazan_kazan_clinic_ru:
							ParseSiteKazanKazanClinicRu(docService, ref itemServiceGroup);
							break;
						default:
							break;
					}
					...
					if (itemServiceGroup.ServiceItems.Count == 0) {
						_backgroundWorker.ReportProgress((int)progressCurrent, App.errorPrefix + "Услуг не обнаружено, пропуск");
						continue;
					}
		...
		private void ParseSiteKazanKazanClinicRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
		...
		private void ParseSiteKrdVrukahCom(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
		...
		private void ParseSiteSpbMedVdkRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup, bool isFirstCycle) {
			if (isFirstCycle) {
			...
			foreach (HtmlNode nodeService in nodeServices) {
				List<ItemService> itemServices = ReadTrNodes(nodeService);
				itemServiceGroup.ServiceItems.AddRange(itemServices);
			}
		}

		private void ParseSiteSpbClinicaBlagodatRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode node in nodeCollection) {
				...
				List<ItemService> services = ReadTrNodes(node, nameOffset, priceOffset);
		...
		private void ParseSiteSpbStarsclinicRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode node in nodeCollection) {
				...
				List<ItemService> services = ReadTrNodes(node, 0, priceOffset);
				itemServiceGroup.ServiceItems.AddRange(services);
			}
		}

		private void ParseSiteSpbDcenergoRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
		...
		private void ParseSiteSpbClinicComplexRu(HtmlDocument docServices, ref ItemServiceGroup itemServiceGroup) {
			...
			HtmlNodeCollection htmlNodesService = _htmlAgility.GetNodeCollection(docServices, xPathService);
			...
			foreach (HtmlNode nodeService in htmlNodesService) {
				...
				ItemService itemService = new ItemService() {
					Name = SiteInfo.ClearString(nodeTitle.InnerText),
					Price = SiteInfo.ClearString(nodePrice.InnerText)
				};
		...
		private void ParseSiteKrdClinicaNazdorovieRuLab(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode nodeTbody in nodeCollectionTbodies) {
				List<ItemService> serviceItems = ReadTrNodes(nodeTbody, 1, 2);
				itemServiceGroup.ServiceItems.AddRange(serviceItems);
			}
		}
		...
		private void ParseSiteKrdClinicaNazdorovieRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode nodeTbody in nodeCollectionTbodies) {
				List<ItemService> serviceItems = ReadTrNodes(nodeTbody, 1, priceOffset);
				itemServiceGroup.ServiceItems.AddRange(serviceItems);
			}
		}
		...
		private void ParseSiteKrdClinic23Ru(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			HtmlNodeCollection nodeCollectionLinksBlocks = _htmlAgility.GetNodeCollection(docService, xPathLinksBlocks);
			...
			HtmlNodeCollection nodeCollectionTbodyDiag = _htmlAgility.GetNodeCollection(docService, xPathTableTbodyDiag);
			...
			HtmlNodeCollection nodeCollectionTbodyAnaliz = _htmlAgility.GetNodeCollection(docService, xPathTableTbodyAnaliz);
			if (nodeCollectionTbodyAnaliz != null) {
				foreach (HtmlNode nodeTbody in nodeCollectionTbodyAnaliz) {
					List<ItemService> serviceItems = ReadTrNodes(nodeTbody, 0, 1);
					if (serviceItems.Count > 0)
						itemServiceGroup.ServiceItems.AddRange(serviceItems);
				}
			}
		...
		private void ParseSiteYekukImmunoresursRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroupRoot) {
			...
			if (nodeCollectionServicesLab != null) {
				...
				if (nodeH3Title != null) {
					...
					if (htmlNodeTbody != null) {
						List<ItemService> serviceItems = ReadTrNodes(htmlNodeTbody, 1, 4);
						if (serviceItems.Count > 0) {
							ItemServiceGroup itemServiceGroup = new ItemServiceGroup() {
								Name = title,
		...
		private void ParseSiteYekukImmunoresursRuGroup(HtmlNodeCollection nodeCollection, ref ItemServiceGroup itemServiceGroupRoot, bool notDefaultOffset = false) {
			foreach (HtmlNode nodeService in nodeCollection) {
				...
				List<ItemService> serviceItems = ReadTrNodes(nodeTbody, offsetName, offsetPrice);
				ItemServiceGroup itemServiceGroupInner = new ItemServiceGroup() {
					Name = groupName,
					Link = itemServiceGroupRoot.Link
				};
		...
		private void ParseSiteYekukRuslabsRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroupRoot) {
			string xPathServices = "//div[@class='sub-service']";
			HtmlNodeCollection nodeCollectionServices = _htmlAgility.GetNodeCollection(docService, xPathServices);
			Console.WriteLine(itemServiceGroupRoot.Name);
			...
			foreach (HtmlNode nodeChild in nodeCollectionServices) {
                ...
                if (nodeGroupName != null) {
                    string nodeInnerText = SiteInfo.ClearString(nodeGroupName.InnerText);
		...
		private void ParseSiteSpbMedswissSpbRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			List<ItemService> serviceItems = ReadTrNodes(nodeCollectionService[0]);
			itemServiceGroup.ServiceItems.AddRange(serviceItems);
		}
		...
		private void ParseSiteSpbGermanClinic(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			List<ItemService> serviceItems = ReadTrNodes(nodeCollectionPrices[0]);
			itemServiceGroup.ServiceItems.AddRange(serviceItems);
		}
		...
		private void ParseSiteSpbBaltzdravRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode nodeInner in nodeCollectionService) {
				...
				if (itemServiceCurrent != null && nodeChild.Name.ToLower().Equals("div")) {
					...
					List<ItemService> serviceItems = ReadTrNodes(nodeCollectionTbody[0], 1, 1);
		...
		private void ParseSiteSpbEvroMedRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode node in nodeCollectionService) {
				...
				List<ItemService> serviceItems = ReadTrNodes(node, offsetName, offsetPrice);
				itemServiceGroup.ServiceItems.AddRange(serviceItems);
			}
		}
		...
		private void ParseSiteSmStomatologyRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			foreach (HtmlNode node in nodeCollectionService) {
				List<ItemService> serviceItems = ReadTrNodes(node);
				itemServiceGroup.ServiceItems.AddRange(serviceItems);
			}
		}
		...
		private void ParseSiteOnClinic(HtmlDocument docServices, ref ItemServiceGroup itemServiceGroup, bool goDeep = true) {
			...
			if (nodeCollectionPriceTable != null) {
				try {
					List<ItemService> serviceItems = ReadTrNodes(nodeCollectionPriceTable[0].ChildNodes[1].ChildNodes[1]);
					itemServiceGroup.ServiceItems = serviceItems;
				} catch (Exception e) {
					Console.WriteLine(e.Message + Environment.NewLine + e.StackTrace);
				}
		...
		private void ParseSiteMedsiRu(HtmlDocument docService, ItemServiceGroup itemServiceGroupRoot) {
			...
			HtmlNodeCollection htmlNodesBladeItem = _htmlAgility.GetNodeCollection(docService, xPathBlades);
			if (htmlNodesBladeItem != null) {
				foreach (HtmlNode nodeBlade in htmlNodesBladeItem) {
					...
					string clinicName = SiteInfo.ClearString(nodeClinicName.InnerText);
					...
					foreach (HtmlNode nodePrice in htmlNodesPrice) {
						...
						ItemService itemService = new ItemService() { Name = serviceName, Price = servicePrice };
						itemServiceGroupInner.ServiceItems.Add(itemService);
					}
		...
		private void ParseSiteDentolRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			List<ItemService> serviceItems = ReadTrNodes(nodeCollectionService.First());
			itemServiceGroup.ServiceItems = serviceItems;
		}
		...
		private void ParseSiteCmdOnlineRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			List<ItemService> serviceItems = ReadTrNodes(nodeCollectionService.First(), 1, 2);
			itemServiceGroup.ServiceItems = serviceItems;
		}
		...
		private void ParseSiteFdoctorRu(HtmlDocument docService, ref ItemServiceGroup itemServiceGroup) {
			...
			try {
				...
				List<ItemService> priceServiceItems = ReadTrNodes(docPrices.DocumentNode);
			...
			foreach (HtmlNode table in nodeCollectionService) {
				...
				List<ItemService> serviceItems = ReadTrNodes(tbody);
				itemServiceGroup.ServiceItems.AddRange(serviceItems);
			}
		}
		...
		private List<ItemService> ReadTrNodes(HtmlNode node, int nameOffset = 0, int priceOffset = 0) {
